...
def start_game(field_width, field_height, block_size, exits):
	...
	field, spawn_x, spawn_y = generators.generate_field_original(field_width, field_height, exits)
...
# просто генерить основной путь максимальной длинны пока не упрёшьсяв тупик, потом от него идти рекурсией ?
# он может добавлять проверки с остальных сторон, просто выбирать не рандомно, а первые элементы,и туда вставлять один из своих рандомных путейб а вот их уже вставлять на рандом
