...
_min_size, _max_size = 10, 500  # Not actually minimum and maximum
...
def generate_action_buttons(menu, on_play, on_back):
	buttons_frame = menu.add.frame_h(250, 100, align=pygame_menu.locals.ALIGN_CENTER)
...
def original_labyrinth_submenu(menu_width, menu_height, on_close, on_back, on_play) -> pygame_menu.Menu:
	...
	# Labyrinth preview
	field, spawn_x, spawn_y = generators.generate_field_original(_default_width, _default_height,
	                                                             _default_exits)
	...
	orig_optimised_menu.add.vertical_margin(10)
	
	generate_again_btn = orig_optimised_menu.add.button('Generate again', action=draw_preview, font_size=40,
	                                                    align=pygame_menu.locals.ALIGN_CENTER)
	...
	generate_action_buttons(orig_optimised_menu,
	                        ...
	                        lambda: on_back(orig_optimised_menu))
...
def customisable_labyrinth_submenu(menu_width, menu_height, on_close, on_back, on_play) -> pygame_menu.Menu:
	def draw_preview():
		...
		centered_spawn = spawn_switch.get_value()
		bfs = bfs_switch.get_value()
	...
	custom_menu.add.vertical_margin(10)
	generate_again_btn = custom_menu.add.button('Generate again', action=draw_preview, font_size=40,
	                                            align=pygame_menu.locals.ALIGN_CENTER)
	...
	l_weight_frame = first_row.pack(custom_menu.add.frame_v(300, _pair_frame_height),
	                                align=pygame_menu.locals.ALIGN_CENTER)
	l_weight_entry = custom_menu.add.text_input('Weight left: ', default=50, input_type="input-int",
	                                            align=pygame_menu.locals.ALIGN_RIGHT)
	l_weight_range = custom_menu.add.range_slider('', default=50, range_values=(1, 100), increment=1, width=150,
	                                              value_format=lambda x: str(round(x)))
	...
	mid_weight_frame = first_row.pack(custom_menu.add.frame_v(300, _pair_frame_height),
	                                  align=pygame_menu.locals.ALIGN_CENTER)
	mid_weight_entry = custom_menu.add.text_input('Weight front: ', default=50, input_type="input-int",
	                                              align=pygame_menu.locals.ALIGN_RIGHT)
	mid_weight_range = custom_menu.add.range_slider('', default=50, range_values=(1, 100), increment=1,
	...
	r_weight_frame = first_row.pack(custom_menu.add.frame_v(300, _pair_frame_height),
	                                align=pygame_menu.locals.ALIGN_CENTER)
	r_weight_entry = custom_menu.add.text_input('Weight right: ', default=50, input_type="input-int",
	                                            align=pygame_menu.locals.ALIGN_RIGHT)
	r_weight_range = custom_menu.add.range_slider('', default=50, range_values=(1, 100), increment=1, width=150,
	                                              value_format=lambda x: str(round(x)))
	...
	new_gen_switch = custom_menu.add.toggle_switch('', default=True, state_text=("Old generator", "New generator"),
	                                               width=235, onchange=lambda _: draw_preview())
	other_frame.pack(new_gen_switch)
	first_row.update_position()
...
def vector_labyrinth_submenu(menu_width, menu_height, on_close, on_back, on_play) -> pygame_menu.Menu:
	def draw_preview():
		...
		reverse = bool(reverse_switch.get_value())
		...
		try:
			field, spawn_x, spawn_y = generators.generate_field_vector(width, height, wall_width, path_width,
			                                                           ...
			                                                           random_node_chance, dead_end_chance,
			                                                           guaranteed_generations, reverse)
		except ValueError:
			preview.fill((255, 0, 0))
			preview.blit(_text, _text_rect)
			return
	...
	vector_menu = pygame_menu.Menu("", menu_width, menu_height, theme=_submenu_theme,
	                               onclose=on_close, verbose=False, mouse_motion_selection=True)
	...
	name_label = vector_menu.add.label("Vector based generator", font_size=50, align=pygame_menu.locals.ALIGN_CENTER)
	desc_label = vector_menu.add.label(desc_text, font_size=30, align=pygame_menu.locals.ALIGN_CENTER)
	vector_menu.add.vertical_margin(40)
	
	# Labyrinth preview
	field, spawn_x, spawn_y = generators.generate_field_custom(_default_width, _default_height, _default_exits, )
	preview = pygame.surface.Surface((_preview_canvas_size, _preview_canvas_size))
	vector_menu.add.surface(preview, align=pygame_menu.locals.ALIGN_CENTER)
	vector_menu.add.vertical_margin(10)
	generate_again_btn = vector_menu.add.button('Generate again', action=draw_preview, font_size=40,
	                                            align=pygame_menu.locals.ALIGN_CENTER)
	vector_menu.add.vertical_margin(20)
	
	# Standard values
	width_entry, height_entry, block_entry, exits_entry = generate_standart_inputs(vector_menu, menu_width,
	                                                                               draw_preview)
	
	# Paths and walls
	first_row = vector_menu.add.frame_h(menu_width - 50, 160)
	first_row.update_position()
	wall_width_frame = first_row.pack(vector_menu.add.frame_v(250, _pair_frame_height),
	                                  align=pygame_menu.locals.ALIGN_CENTER)
	wall_width_entry = vector_menu.add.text_input('Wall width: ', default=1, input_type="input-int",
	                                              align=pygame_menu.locals.ALIGN_RIGHT)
	wall_width_range = vector_menu.add.range_slider('', default=1, range_values=(1, 20), increment=1, width=50,
	                                                value_format=lambda x: str(round(x)))
	configure_entry_range_pair(wall_width_entry, wall_width_range, wall_width_frame, 1, 20, draw_preview)
	first_row.update_position()
	
	path_width_frame = first_row.pack(vector_menu.add.frame_v(250, _pair_frame_height),
	                                  align=pygame_menu.locals.ALIGN_CENTER)
	path_width_entry = vector_menu.add.text_input('Path width: ', default=1, input_type="input-int",
	                                              align=pygame_menu.locals.ALIGN_RIGHT)
	path_width_range = vector_menu.add.range_slider('', default=1, range_values=(1, 20), increment=1,
	...
	min_path_len_frame = first_row.pack(vector_menu.add.frame_v(400, _pair_frame_height),
	                                    align=pygame_menu.locals.ALIGN_CENTER)
	min_path_len_frame._relax = True
	min_path_len_entry = vector_menu.add.text_input('Minimum path length: ', default=1, input_type="input-int",
	                                                align=pygame_menu.locals.ALIGN_RIGHT)
	min_path_len_range = vector_menu.add.range_slider('', default=1, range_values=(1, 20), increment=1, width=50,
	                                                  value_format=lambda x: str(round(x)))
	configure_entry_range_pair(min_path_len_entry, min_path_len_range, min_path_len_frame, 1, 20, draw_preview)
	first_row.update_position()
	
	max_path_len_frame = first_row.pack(vector_menu.add.frame_v(400, _pair_frame_height),
	                                    align=pygame_menu.locals.ALIGN_CENTER)
	max_path_len_frame._relax = True
	max_path_len_entry = vector_menu.add.text_input('Maximum path length: ', default=100, input_type="input-int",
	                                                align=pygame_menu.locals.ALIGN_RIGHT)
	max_path_len_range = vector_menu.add.range_slider('', default=100, range_values=(2, 100), increment=1, width=150,
	                                                  value_format=lambda x: str(round(x)))
	configure_entry_range_pair(max_path_len_entry, max_path_len_range, max_path_len_frame, 1, 100, draw_preview)
	first_row.update_position()
	
	# Node selection
	second_row = vector_menu.add.frame_h(menu_width - 50, 160)
	second_row.update_position()
	
	random_node_frame = second_row.pack(vector_menu.add.frame_v(400, _pair_frame_height),
	                                    align=pygame_menu.locals.ALIGN_CENTER)
	random_node_entry = vector_menu.add.text_input('Random node chance: ', default=0, input_type="input-int",
	                                               align=pygame_menu.locals.ALIGN_RIGHT)
	random_node_range = vector_menu.add.range_slider('', default=0, range_values=(0, 100), increment=1,
	                                                 width=_range_width, value_format=lambda x: str(round(x)))
	configure_entry_range_pair(random_node_entry, random_node_range, random_node_frame, 0, 100, draw_preview)
	second_row.update_position()
	
	dead_end_frame = second_row.pack(vector_menu.add.frame_v(400, _pair_frame_height),
	                                 align=pygame_menu.locals.ALIGN_CENTER)
	dead_end_entry = vector_menu.add.text_input('Dead end chance: ', default=0, input_type="input-int",
	                                            align=pygame_menu.locals.ALIGN_RIGHT)
	dead_end_range = vector_menu.add.range_slider('', default=0, range_values=(0, 100), increment=1, width=_range_width,
	                                              value_format=lambda x: str(round(x)))
	configure_entry_range_pair(dead_end_entry, dead_end_range, dead_end_frame, 0, 100, draw_preview)
	second_row.update_position()
	
	guaranteed_frame = second_row.pack(vector_menu.add.frame_v(500, _pair_frame_height),
	                                   align=pygame_menu.locals.ALIGN_CENTER)
	guaranteed_entry = vector_menu.add.text_input('Guaranteed generations: ', default=10, input_type="input-int",
	                                              align=pygame_menu.locals.ALIGN_RIGHT)
	guaranteed_range = vector_menu.add.range_slider('', default=10, range_values=(0, 1000), increment=1,
	                                                width=350, value_format=lambda x: str(round(x)))
	configure_entry_range_pair(guaranteed_entry, guaranteed_range, guaranteed_frame, 0, 1000, draw_preview)
	second_row.update_position()
	
	# Other
	third_row = vector_menu.add.frame_h(menu_width - 50, 160)
	third_row.update_position()
	toggles_frame = third_row.pack(vector_menu.add.frame_v(300, _pair_frame_height),
	                               align=pygame_menu.locals.ALIGN_CENTER)
	
	check_angle_step_frame = third_row.pack(vector_menu.add.frame_v(500, _pair_frame_height),
	                                        align=pygame_menu.locals.ALIGN_CENTER)
	check_angle_step_entry = vector_menu.add.text_input('Check angle step: ', default=30, input_type="input-int",
	                                                    align=pygame_menu.locals.ALIGN_RIGHT)
	check_angle_step_range = vector_menu.add.range_slider('', default=30, range_values=(1, 359), increment=1,
	                                                      width=350, value_format=lambda x: str(round(x)))
	configure_entry_range_pair(check_angle_step_entry, check_angle_step_range, check_angle_step_frame, 1, 359,
	                           draw_preview)
	third_row.update_position()
	
	ignore_tiles_rad_frame = third_row.pack(vector_menu.add.frame_v(500, _pair_frame_height),
	                                        align=pygame_menu.locals.ALIGN_CENTER)
	ignore_tiles_rad_entry = vector_menu.add.text_input('Ignore tiles radius: ', default=3, input_type="input-int",
	                                                    align=pygame_menu.locals.ALIGN_RIGHT)
	ignore_tiles_rad_range = vector_menu.add.range_slider('', default=3, range_values=(0, 10), increment=1,
	                                                      width=50, value_format=lambda x: str(round(x)))
	...
	spawn_switch = vector_menu.add.toggle_switch('', default=False, state_text=("Random start", "Start center"),
	                                             width=235, onchange=lambda _: draw_preview())
	toggles_frame.pack(spawn_switch)
	
	reverse_switch = vector_menu.add.toggle_switch('', default=False, state_text=("Not reversed", "Reversed"),
	                                               width=210, onchange=lambda _: draw_preview())
	toggles_frame.pack(reverse_switch)
	
	bfs_switch = vector_menu.add.toggle_switch('', default=False, state_text=("DFS", "BFS"), width=100,
	                                           onchange=lambda _: draw_preview())
	...
	# Buttons
	vector_menu.add.vertical_margin(20)
	generate_action_buttons(vector_menu,
	                        lambda: on_play(field, spawn_x, spawn_y, int(block_entry.get_value()), vector_menu),
	                        lambda: on_back(vector_menu))
